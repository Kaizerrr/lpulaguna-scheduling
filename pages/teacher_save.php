	...
	$status = $_POST['status'];	
		...
		$query=mysqli_query($con,"select * from member where member_last='$last' and  member_first='$first'")or die(mysqli_error());
				...
				if ($count>0)
				{
					...
					mysqli_query($con,"INSERT INTO member(member_salut,member_last,member_first,dept_code,designation_id,username,password,status) 
					VALUES('$salut','$last','$first','$dept','$designation','$username','$password','$status')")or die(mysqli_error());
