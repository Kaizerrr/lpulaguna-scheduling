...
include('../dist/includes/dbcon.php');
	...
	foreach ($s as $days){
		//check conflict
		$query=mysqli_query($con,"select *,COUNT(*) as count from schedule 
		natural join teacher natural join time where teacher_id='$teacher' and room='$room' and day='s' and schedule.time_id='$days'")or die(mysqli_error($con));
			$row=mysqli_fetch_array($query);
			$count=$row['count'];
			$time1=date("h:i a",strtotime($row['time_start']))."-".date("h:i a",strtotime($row['time_end']));
			$teacher1=$row['teacher_last'].", ".$row['teacher_first'];
		...
		$querytime=mysqli_query($con,"select * from time where time_id='$days'")or die(mysqli_error($con));
				$rowt=mysqli_fetch_array($querytime);
				$timet=date("h:i a",strtotime($rowt['time_start']))."-".date("h:i a",strtotime($rowt['time_end']));	
		...
		{
			mysqli_query($con,"INSERT INTO schedule(time_id,day,teacher_id,subject_code,cys,room,remarks) 
				VALUES('$days','s','$teacher','$subject','$cys','$room','$remarks')")or die(mysqli_error());
				
			echo "<span class='text-success'>$timet $teachert $cys at room $room every Saturday successfully added!</span><br>";	
		}			
		else{
			
			echo "<span class='text-danger'>$time1 is already taken by $teacher1 $cys1 at room $room1 every saturday </span><br>";
